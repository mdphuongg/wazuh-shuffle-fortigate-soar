...
```bash
https://<SERVER-IP>:3001/api/v1/hooks/<HOOK_ID>
```
...
<integration>
  ...
  <level><MINIMUM_ALERT_LEVEL></level>
  <rule_id><TRIGGER_RULE_IDS></rule_id>
...
|-----------|-------------|
| hook_url | Shuffle webhook endpoint |
| level | Minimum alert severity |
| rule_id | Wazuh rules triggering automation |
| alert_format | Alert format sent to Shuffle |
